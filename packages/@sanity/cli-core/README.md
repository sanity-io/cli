...
Instead of importing from the main `@sanity/cli-core` barrel (which pulls in the full dependency graph, including `jsdom`), prefer importing from the relevant sub-path.

> **Note:** The root `@sanity/cli-core` export is kept for backwards compatibility. Do not add new exports there; use the appropriate sub-path instead.
...
Browser environment mock. Isolated to its own export to avoid pulling in `jsdom` (~46 MB) unnecessarily.
...
Pure, zero-dependency utility functions.
