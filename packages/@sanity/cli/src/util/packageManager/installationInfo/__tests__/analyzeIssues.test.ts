...
describe('analyzeIssues', () => {
  ...
  test('declared-not-installed suggests bun install for standalone bun project without lockfile', () => {
    const bunWorkspace: WorkspaceInfo = {
      bunfig: true,
      hasMultipleLockfiles: false,
      lockfile: null,
      nearestPackageJson: '/project/package.json',
    ...
    const issues = analyzeIssues(packages, bunWorkspace, [])

    const issue = issues.find((i) => i.type === 'declared-not-installed')
    expect(issue).toBeDefined()
    expect(issue?.suggestion).toBe('Run: bun install')
  })
  ...
  test('treats pinned cliDependencyRange as caret range for local cli-version-incompatible check', () => {
    const packages: Partial<Record<'@sanity/cli' | 'sanity', PackageInfo>> = {
      '@sanity/cli': {
        declared: null,
        installed: {
          cliDependencyRange: null,
          path: '/project/node_modules/@sanity/cli',
          version: '5.32.0', // Below 5.33.0 but same major — satisfies ^5.33.0? No.
        },
        override: null,
      },
      sanity: {
    ...
    // 5.32.0 does NOT satisfy ^5.33.0, so should flag as incompatible
    const issue = issues.find((i) => i.type === 'cli-version-incompatible')
    expect(issue).toBeDefined()
    expect(issue?.message).toContain('^5.33.0')
  })

  test('treats pinned cliDependencyRange as caret range for global compatibility check', () => {
    const packages: Partial<Record<'@sanity/cli' | 'sanity', PackageInfo>> = {
      sanity: {
        declared: {
          declaredVersionRange: '^3.67.0',
  ...
  test('does not double-report @sanity/cli as both declared-not-installed and conflicting-cli-dependency', () => {
    // When @sanity/cli is declared with an incompatible version AND not installed,
    // only the CLI-specific conflicting-cli-dependency should fire — not declared-not-installed.
    const packages: Partial<Record<'@sanity/cli' | 'sanity', PackageInfo>> = {
      '@sanity/cli': {
        declared: {
          declaredVersionRange: '^4.0.0',
          dependencyType: 'dependencies',
          packageJsonPath: '/project/package.json',
          versionRange: '^4.0.0', // Incompatible with sanity's ^5.33.0
        },
        installed: null, // Not installed
        override: null,
      },
    ...
    // Should get conflicting-cli-dependency but NOT declared-not-installed
    expect(issues.filter((i) => i.type === 'conflicting-cli-dependency')).toHaveLength(1)
    expect(issues.filter((i) => i.type === 'declared-not-installed')).toHaveLength(0)
  })

  test('does not double-report @sanity/cli as both declared-not-installed and redundant-cli-dependency', () => {
    const packages: Partial<Record<'@sanity/cli' | 'sanity', PackageInfo>> = {
      '@sanity/cli': {
        declared: {
          declaredVersionRange: '^5.33.0',
          dependencyType: 'dependencies',
          packageJsonPath: '/project/package.json',
          versionRange: '^5.33.0', // Compatible — redundant
        },
        installed: null, // Not installed
        override: null,
      },
    ...
    // Should get redundant-cli-dependency but NOT declared-not-installed
    expect(issues.filter((i) => i.type === 'redundant-cli-dependency')).toHaveLength(1)
    expect(issues.filter((i) => i.type === 'declared-not-installed')).toHaveLength(0)
  })

  test('still reports declared-not-installed for @sanity/cli when sanity has no cliDependencyRange', () => {
    // If sanity is not installed or has no cliDependencyRange, the CLI section won't run,
    // so declared-not-installed should still fire as a fallback.
    const packages: Partial<Record<'@sanity/cli' | 'sanity', PackageInfo>> = {
      '@sanity/cli': {
        declared: {
          declaredVersionRange: '^5.33.0',
    ...
    expect(issues.filter((i) => i.type === 'declared-not-installed')).toHaveLength(1)
  })
