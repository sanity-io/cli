...
import {debug as baseDebug} from '../debug.js'
import {getClientWrapper, getCliToken} from './clientWrapper.js'
...
function createTelemetryClient(token: string): SanityClient {
  const getClient = getClientWrapper(null, 'sanity.cli.js')
  const baseClient = getClient({requireUser: false, requireProject: false})
  const configured = baseClient.withConfig({
  ...
  return configured as any as SanityClient
}

let _client: SanityClient | null = null
function getCachedClient(token: string): SanityClient {
  if (!_client) {
