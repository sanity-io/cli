...
import {findNdjsonEntry, readNdjsonFile} from '../findNdjsonEntry.js'
...
describe('findNdjsonEntry', () => {
  ...
  it('throws an error if invalid JSON is encountered before match', async () => {
    ...
    try {
      ...
      for await (const _match of findNdjsonEntry(invalidStream(), () => false)) {
        // Intentionally empty - we expect this to throw
      }
    } catch {
      hasThrown = true
    }
