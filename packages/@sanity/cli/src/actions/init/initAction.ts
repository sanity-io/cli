...
async function getProjectDetails({
  ...
  if (isAppTemplate) {
    let organizationId: string | undefined = organization
    if (!organizationId) {
      let organizations: ProjectOrganization[]
      try {
      ...
      organizationId = await promptUserForOrganization({
        isAppTemplate: true,
        organizations,
        user,
      })
    ...
    const {datasetName, displayName, projectId} = await promptForAppTemplateSetup({
      dataset,
      datasetDefault,
      newProject,
      organizationId,
...
async function promptForAppTemplateSetup({
  dataset,
  datasetDefault,
  newProject,
  organizationId,
