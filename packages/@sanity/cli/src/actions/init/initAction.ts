...
export async function initAction(options: InitOptions, context: InitContext): Promise<void> {
  ...
  if (options.unattended) {
    checkFlagsInUnattendedMode(options, isNextJs)
  }
  ...
  const isAppTemplate = options.template ? determineAppTemplate(options.template) : false // Default to false
  if (!isAppTemplate) {
  ...
  const mcpResult = await setupMCP({skip: !options.mcp})
...
function checkFlagsInUnattendedMode(options: InitOptions, isNextJs: boolean): void {
