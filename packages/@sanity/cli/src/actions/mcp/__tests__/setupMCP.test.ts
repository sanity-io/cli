...
describe('setupMCP', () => {
  ...
  test('continues writing other editors when one write fails, reporting the first error', async () => {
    const cursor = {authStatus: 'unknown', configured: false, name: 'Cursor'}
    const vscode = {authStatus: 'unknown', configured: false, name: 'VS Code'}
    mockDetectAvailableEditors.mockResolvedValue([cursor, vscode])
    mockValidateEditorTokens.mockResolvedValue(undefined)
    mockPromptForMCPSetup.mockResolvedValue([cursor, vscode])
    mockCreateMCPToken.mockResolvedValue('test-token')
    // Cursor fails, VS Code succeeds — the loop must not abort on the first failure.
    mockWriteMCPConfig
    ...
    expect(result.error?.message).toBe('cursor write failed')
    expect(result.skipped).toBe(false)
  })

  test('reports an error and skips the editor when removal fails, still processing writes', async () => {
    const cursor = {
      authStatus: 'valid',
      configured: true,
      existingToken: 'existing-token',
    ...
    mockDetectAvailableEditors.mockResolvedValue([cursor, vscode])
    mockValidateEditorTokens.mockResolvedValue(undefined)
    // User keeps VS Code, drops the configured Cursor — removal of Cursor fails.
    mockPromptForMCPSetup.mockResolvedValue([vscode])
    mockWriteMCPConfig.mockResolvedValue(true)
    mockRemoveMCPConfig.mockRejectedValue(new Error('cursor remove failed'))
    ...
    expect(mockRemoveMCPConfig).toHaveBeenCalledWith(cursor)
    expect(result.configuredEditors).toEqual(['VS Code'])
    // Removal failed, so Cursor must not be reported as removed.
    expect(result.removedEditors).toEqual([])
    expect(result.error).toBeInstanceOf(Error)
    expect(result.error?.message).toBe('cursor remove failed')
  })
})
