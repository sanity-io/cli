...
import {type AssetWithAspects, resolveSource, setAspects} from '../importMedia.js'
...
describe('resolveSource', () => {
  it('accepts a directory source', async () => {
    const [result] = await lastValueFrom(
      resolveSource({
        sourcePath: path.resolve(__dirname, './fixtures/test-media-library-directory'),
      }).pipe(toArray()),
    )
  ...
  it('accepts a tar.gz source', async () => {
    const [result] = await lastValueFrom(
      resolveSource({
        sourcePath: path.resolve(__dirname, './fixtures/test-media-library-archive.tar.gz'),
      }).pipe(toArray()),
    )
