...
describe('#downloadStream', () => {
  ...
  test('streams the response body into the destination', async () => {
    ...
    expect(mock.getRequests()[0]).toHaveHeader(
      'user-agent',
      expect.stringContaining('@sanity/cli-core@'),
    )
  ...
  test('aborts the response when no data arrives before the read timeout', async () => {
    ...
    expect(body.lastAbortReason).toMatchObject({
      message: 'Backup download stalled: no data received for 3 minutes. Try again.',
    })
  })
})
