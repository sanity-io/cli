...
describe('#buildStudio', () => {
  ...
  describe('auto-updates on a federated studio', () => {
    test('builds pinned, without an import map', async () => {
      const output = createMockOutput()

      await buildStudio(buildOptions({autoUpdatesEnabled: true, isWorkbenchApp: true, output}))

      expect(mockedBuildStaticFiles).toHaveBeenCalledWith(
        expect.objectContaining({autoUpdates: undefined}),
      )
    })

    test('skips the whole preamble, so no import map is resolved and appId is not checked', async () => {
      const output = createMockOutput()
      const mockCheckAppId = vi.fn()
      ...
      expect(mockCheckAppId).not.toHaveBeenCalled()
      expect(getAutoUpdatesImportMap).not.toHaveBeenCalled()
      expect(getAutoUpdatesCssUrls).not.toHaveBeenCalled()
    })

    test('never prompts about version drift, since auto-updates do not apply', async () => {
      const output = createMockOutput()
      const onVersionMismatchInInteractiveAutoUpdate = vi.fn()
      mockedIsInteractive.mockReturnValue(true)
    ...
    test('does not announce auto-updates in the build log', async () => {
      const output = createMockOutput()

      await buildStudio(buildOptions({autoUpdatesEnabled: true, isWorkbenchApp: true, output}))

      expect(output.log).not.toHaveBeenCalledWith(
        expect.stringContaining('Building with auto-updates enabled'),
      )
    })
  })
})
